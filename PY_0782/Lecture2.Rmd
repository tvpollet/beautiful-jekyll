...
* calculate 'airtime_hours' which is airtime (in mins.) divide by the number of minutes in an hour. 
...
```{r}{r, fig.height=1.8, fig.width=3, tidy.opts=list(width.cutoff=35)}
...
violinplot + xlab("Carrier")+ylab("Delay Departure (mins.)") + theme_few(14)
```
...
### violinplot_2
violinplot_2<-ggplot(violinplot_data, aes(x = carrier_rec, y = dep_delay)) + geom_violin() + geom_boxplot(width = 0.2)  + xlab("Carrier")+ylab("Delay Departure (mins.)") + theme_few(14)
violinplot_2
```
...
*  Wilcoxon-Mann-Whitney U is common [alternative](http://vassarstats.net/textbook/ch11a.html) to _t_test. (Sometimes, just called Mann-Whitney U).
```{r}
...
## Exercise. 
- Install and load the 'car' package. load the professorial 'Salaries' dataset.
