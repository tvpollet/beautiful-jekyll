...
An independent-samples _t_-test was used to compare the (mean) monthly salaries between men and women. There was a significant difference with men (_M_ = \$12787, _SD_ = 3381.88) earning on average more than women do (_M_ = \$11222, _SD_= 2883.57), _t_(50.12)= 3.16, _p_=.003.
...
The 95% bootstrapped confidence interval for _p_ ranges from <.0001 to .2872. (But note if you had picked a smaller CI, 90%CI, it would be a narrower range, duh :) - still quite a broad range)

### Median test

```{r}
wilcox.test(monthly_salary~gender, data=salaries)
gender_group_med<- salaries %>% group_by(gender) %>% summarise(SD_monthly=median(monthly_salary))
gender_group_med
```

A Wilcoxon-Mann-Whitney U test showed that men (_Median_=12004) earned significantly more than women did (_Median_=11527) _W_=5182.5, _p_= .008.
...
```
The modal monthly income for men was \$9222 for women it was \$8333.
