...
Note that if you have sampled the entire population
...
```{r, warning=F, message=F, tidy.opts=list(width.cutoff=12), tidy=T, fig.align="center",fig.height=3, fig.width=8}
require(ggplot2) # note geom_jitter
require(ggthemes)
require(RColorBrewer)
ggplot(data = iris,aes(Sepal.Width, Sepal.Length, color=Species)) + geom_jitter() + facet_grid(. ~ Species) + geom_smooth(method='lm') + scale_colour_brewer(palette = "Dark2") + labs(x="Sepal Width", y="Sepal Length", title="Sepals ...") + theme_tufte(12) 
```
...
```{r, warning=F, message=F, tidy.opts=list(width.cutoff=12), tidy=T, fig.align="center",fig.height=3.5, fig.width=8}
...
Remember scale() - $\beta$'s are nothing more than standardized B's! (but beware scale() takes matrices as input).
...
Note be wary with interaction terms! (center them beforehand - values might not be correct from QuantPsyc). Alternative package lm.beta.
...
return(B_boot$coeff[2]) # education in position 2 
}
